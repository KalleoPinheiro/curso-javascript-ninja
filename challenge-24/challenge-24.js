((doc) => {
  ...
  var $buttonsNumbers = doc.querySelectorAll(
  ...
  function getOperations() {
    return Array.prototype.map.call($buttonsOperations, (operator) =>
      operator.value);
  }
  ...
  function getRegexOperations() {
    return new RegExp('\\d+[' + getOperations().toString() + ']?', 'g');
  }
  ...
  function calculateValues(accumulated, actual) {
    ...
    return chooseOperator(operator, firstValue, lastValue) + lastOperator;
  }
  ...
  function chooseOperator(operator, firstValue, lastValue) {
    switch (operator) {
      case '+':
